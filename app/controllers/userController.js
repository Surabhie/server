...
let getAllUser = (req, res) => {
    ...
    UserModel.find()
        .select(' -__v -_id') //Anything you want to hide from the response can be done like this.Then mongoose wont show it in the response
        .lean() //Anything we want from mongoose,to make it a plain javascript obj
        .exec((err, result) => { //exec contains the callback function that is needed
            if (err) {
                console.log(err)
                logger.error(err.message, 'User Controller: getAllUser', 10) //captureError
    ...
    UserModel.findOne({ 'userId': req.params.userId })
        ...
        .exec((err, result) => {
            if (err) {
                ...
                let apiResponse = response.generate(true, 'No User Found', 404, null) //response library
                res.send(apiResponse)
            } else {
...
let signUpFunction = (req, res) => {
        ...
        return new Promise((resolve, reject) => {
            if (req.body.email) {
                if (!validateInput.Email(req.body.email)) {
                    let apiResponse = response.generate(true, 'Email should meet the requirement', 400, null)
                    reject(apiResponse)
                } else if (check.isEmpty(req.body.password)) {
                    let apiResponse = response.generate(true, 'your password please', 400, null)
                    reject(apiResponse)
                } else {
                    resolve(req)
                }
            } else {
                logger.error('Fill all the details', 'userController: createUser()', 5)
            ...
            UserModel.findOne({ email: req.body.email })
                .exec((err, retrievedUserDetails) => {
                    if (err) {
                        ...
                        let apiResponse = response.generate(true, 'Failed to create the User', 500, null)
                        reject(apiResponse)
                    } else if (check.isEmpty(retrievedUserDetails)) {
                        console.log(req.body)
                        let newUser = new UserModel({ //creating instance of the UserModel
                            userId: shortid.generate(),  //npm install shortid required
                        ...
                        newUser.save((err, newUser) => { //save function by default is in the mongoose
                            if (err) {
                                console.log(err)
                                logger.error(err.message, 'userController: createUser', 10)
                        ...
                        logger.error('This user is already Present.Therefore cannot be created again', 'userController: createUser', 4)
                        let apiResponse = response.generate(true, 'User with this Email Id is already present', 403, null)
                        reject(apiResponse)
                    }
                })
        })
    ...
    validateUserInput(req, res)
        ...
        .then((resolve) => {
            delete resolve.password //password should not be visible to anyone.So before generating
            // apiResponse needs to be deleted
...
let loginFunction = (req, res) => {
            ...
            if (req.body.email) {
                ...
                UserModel.findOne({ email: req.body.email }, (err, userDetails) => {
                    ...
                    if (err) {
                        ...
                        /* generate the error message and the api response message  */
                        let apiResponse = response.generate(true, 'Failed To Find User Details', 500, null)
                        reject(apiResponse)
                        /* iff user details  not found */
                    } else if (check.isEmpty(userDetails)) {
                        /* generate the response and the console error message */
                        logger.error('No User Found', 'userController: findUser()', 7)
                        let apiResponse = response.generate(true, 'No User Details Found', 404, null)
                        reject(apiResponse)
                    } else {
                        /* the message and the api response  */
        ...
        return new Promise((resolve, reject) => {
            passwordLib.comparePassword(req.body.password, retrievedUserDetails.password, (err, isMatch) => {
                if (err) {
                    ...
                    logger.info('Login is failed due to an invalid password', 'userController: validatePassword()', 10)
                    let apiResponse = response.generate(true, 'Password is incorrect.Login Failed', 400, null)
                    reject(apiResponse)
                }
            })
        })
    ...
    let generateToken = (userDetails) => {
        console.log("generate token");
        return new Promise((resolve, reject) => {
            token.generateToken(userDetails, (err, tokenDetails) => { //token library
                if (err) {
                    console.log(err)
                    let apiResponse = response.generate(true, 'Failed To Generate Token', 500, null)
                    reject(apiResponse)
        ...
        return new Promise((resolve, reject) => {
            AuthModel.findOne({ userId: tokenDetails.userId }, (err, retrievedTokenDetails) => {
                if (err) {
                    ...
                    let newAuthToken = new AuthModel({ //instance of the AuthModel
                        userId: tokenDetails.userId,
                        authToken: tokenDetails.token,
                        tokenSecret: tokenDetails.tokenSecret,
                        tokenGenerationTime: time.now()
...
let resetPasswordFunction = (req, res) => {
            ...
            UserModel.update({ 'email': req.body.email }, options).exec((err, result) => {
                if (err) {
                    ...
                    let sendEmailOptions = {
                        ...
                        html: `<h1> Hello ${tokenDetails.userDetails.firstName}</h1>
                            <p>
                                We have received a request to reset your password associated with this ${tokenDetails.userDetails.email} . <br>
                                <br>Kindly click on the link below that has been provided to you for the password reset. <br>
                                <br> <a href="${applicationUrl}/reset-password/${options.validationToken}">Link</a>                                 
                            </p>
    
                            <br><b>Issue Tracker association</b>
                                        `
                    }
    ...
    findUser(req, res)
        ...
        .then(resetPassword)
        .then((resolve) => {
            let apiResponse = response.generate(false, 'Instruction for password reset has been sent successfully', 200, 'None')
            res.status(200)
            res.send(apiResponse)
        })
        .catch((err) => {
            ...
            UserModel.update({ 'userId': userDetails.userId }, options).exec((err, result) => {
                if (err) {
                    ...
                    let sendEmailOptions = {
                        ...
                        html: `<h4> Hello ${userDetails.firstName}</h4>
                        <p>
                            Your password is updated successfully.
                        </p>
                        <h3> Thanks for being a part of Issue Tracker </h3>
                                    `
                    }
