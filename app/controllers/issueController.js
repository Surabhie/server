...
let getIssueDetailsById = (req, res) => {
...
let createIssue = (req, res) => {
    ...
    if (req.file) {
        ...
        let apiResponse = response.generate(true, 'Please make sure an image is selected', 500, null)
        res.send(apiResponse)
...
let searchIssue = (req, res) => {
    IssueModel.count().exec((err, result) => {
        if (err) {
            ...
            /* Search issue by description or title */
...
// Pushing all the comments to comment array
...
addWatchee = (req, res) => {
    req.body.watching = JSON.parse(req.body.watching) // watching comes from issue model
...
let deleteIssue = (req, res) => {
    ...
    IssueModel.findOneAndRemove({ 'issueId': req.params.issueId })
...
module.exports = {
    ...
    getIssueDetailsById: getIssueDetailsById,
